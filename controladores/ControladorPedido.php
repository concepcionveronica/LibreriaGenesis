...
require_once("Conexion.php");
	...
	if (isset($_POST['eliminar_datos']) && $_POST['eliminar_datos']=="si_eliminalos") {
		...
		try {
			$sql = "INSERT INTO pedido(fecha_rec,cantidad,cliente_idCliente)values(?,?,?) producto()";
		...
		foreach ($datos as $row){
			...
			$html_td.='<tr>';
				...
				$html_td.='<td>
								<button  type="button" class="btn btn-primary"  data-id="'.$row['dui'].'"><img src="./public/imagenes/agregar.png" height="40" width="40" onclick=""class="rounded-circle"></button>
